...
import type {
  ...
  InvoiceTemplate, PaymentReminder,
} from '../types';
...
export default function Invoices({ t, language }: InvoicesProps) {
  ...
  const [reminderTarget, setReminderTarget] = useState<InvoiceRecord | null>(null);
  const [reminderLevelTarget, setReminderLevelTarget] = useState(1);
  const { sendReminder, sending: sendingReminder } = useSendReminder();
  ...
  const fetchAllReminders = useCallback(async () => {
    try {
      ...
      setAllReminders((data as PaymentReminder[]) || []);
    } catch {
      // non-critical
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchAllReminders();
  }, [fetchInvoices, fetchClientsAndVehicles, fetchAllReminders]);
  ...
  async function handleSendReminder() {
    ...
    try {
      await sendReminder(reminderTarget.id, reminderLevelTarget, username || 'App');
      toast.success(t('rem.sent'));
      setReminderTarget(null);
      await fetchAllReminders();
    } catch (err) {
      toast.error('Napaka pri pošiljanju opomnika: ' + (err as Error).message);
    }
  }
    ...
    <div className="space-y-5 animate-fadeIn">
              ...
              <tbody>
                {paginatedInvoices.map((inv) => (
                  ...
                  >
                    ...
                    <td className="table-cell">
                      {inv.status === 'sent' && (() => {
                        const rems = remindersByInvoice.get(inv.id) ?? [];
                        const nextLevel = rems.length === 0
                          ? 1
                          : Math.min(Math.max(...rems.map((r) => r.reminder_level)) + 1, 5);
                        const isDisabled = nextLevel > 4;
                        return (
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              onClick={() => {
                                ...
                                setReminderLevelTarget(nextLevel);
                              }}
                              title={isDisabled ? t('rem.all_sent') : (REMINDER_LEVEL_NAMES[nextLevel] ?? '')}
                              className={`flex items-center gap-1 p-1.5 rounded text-xs font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isDisabled ? 'bg-gray-100 text-gray-400' : (REMINDER_BUTTON_COLORS[nextLevel] ?? 'bg-primary text-white hover:bg-primary-hover')}`}
                            >
                              <Bell size={12} strokeWidth={2} />
                              {!isDisabled && <span>{nextLevel}</span>}
                            </button>
                            {rems.length > 0 && (
                              <ReminderHistoryPopover reminders={rems}>
                                <span className="w-5 h-5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold flex items-center justify-center cursor-pointer hover:bg-amber-200 transition-colors">
                                  {rems.length}
                                </span>
      ...
      {reminderTarget && (
        <ReminderConfirmModal
          ...
          reminderLevel={reminderLevelTarget}
